		...
		IsInBackground(false)
{
	if (++SDLDeviceInstances == 1) {
		...
		SDL_SetHint(SDL_HINT_APP_NAME, CreationParams.ApplicationName);
#endif
...
bool CIrrDeviceSDL::run()
{
	...
	while (!Close && wrap_PollEvent(&SDL_event)) {
		...
		case SDL_EVENT_MOUSE_BUTTON_UP: {
			...
			if (irrevent.EventType == EET_MOUSE_INPUT_EVENT && irrevent.MouseInput.Event != irr::EMIE_MOUSE_MOVED) {
				irrevent.MouseInput.ButtonStates = MouseButtonStates;
...
void CIrrDeviceSDL::CCursorControl::initCursors()
{
	...
	Cursors.emplace_back(nullptr);                                               // ECI_UP                                          // ECI_UP
}
